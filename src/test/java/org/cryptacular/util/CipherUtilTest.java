...
public class CipherUtilTest {
    ...
    public Object[][] getAeadBlockCipherData() {
        return new Object[][] {
            ...
            new Object[] {
                // Plaintext is multiple of block size
                "Cogito ergo sum.", GCMBlockCipher.newInstance(AESEngine.newInstance()),
            },
            // CCM
            new Object[] {
    ...
    public void testBlockCipherEncryptDecrypt(
            final String plaintext, final BlockCipher cipher, final Nonce nonce) {
    ...
    public void testAeadBlockCipherEncryptDecrypt(
            final String plaintext, final AEADBlockCipher cipher) {
        final BlockCipher under = cipher.getUnderlyingCipher();
        final SecretKey key = SecretKeyGenerator.generate(under);
        final byte[] ciphertext =
                CipherUtil.encrypt(cipher, key, new RBGNonce(12), plaintext.getBytes());
        final byte[] result = CipherUtil.decrypt(cipher, key, ciphertext);
        assertEquals(new String(result), plaintext);
    }
    ...
    public void testDecryptArrayBackwardCompatibleHeader() {
        ...
        final byte[] plaintext =
                CipherUtil.decrypt(cipher, STATIC_KEY, CodecUtil.hex(v1CiphertextHex));
