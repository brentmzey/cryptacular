...
public class CertUtilTest {
    ...
    public Object[][] getSubjectDN() {
        return new Object[][] {
            new Object[] {
                CertUtil.readCertificate(CRT_PATH + "ed.middleware.vt.edu.crt"),
                "C=US,DC=edu,DC=vt,ST=Virginia,L=Blacksburg,O=Virginia Polytechnic Institute and"
                        + " State University,OU=Middleware-Server-with-saltr,OU=Middleware"
                        + " Services,CN=ed.middleware.vt.edu",
            },
        };
    }
    ...
    public Object[][] getSubjectDNWithSpaces() {
        return new Object[][] {
            new Object[] {
                CertUtil.readCertificate(CRT_PATH + "ed.middleware.vt.edu.crt"),
                "C=US, DC=edu, DC=vt, ST=Virginia, L=Blacksburg, O=Virginia Polytechnic Institute"
                        + " and State University, OU=Middleware-Server-with-saltr, OU=Middleware"
                        + " Services, CN=ed.middleware.vt.edu",
            },
        };
    }
    ...
    public Object[][] getP7EncodedCert() throws Exception {
        return new Object[][] {
            new Object[] {
                ...
                new String(
                        Files.readAllBytes(
                                new File(CRT_PATH + "ed.middleware.vt.edu.p7b").toPath())),
            },
        };
    }
    ...
    public Object[][] getX509Cert() throws Exception {
        return new Object[][] {
            new Object[] {
                ...
                new String(
                        Files.readAllBytes(
                                new File(CRT_PATH + "ed.middleware.vt.edu.crt").toPath())),
            },
        };
    }
    ...
    public Object[][] getBasicUsage() throws Exception {
        return new Object[][] {
            new Object[] {
                CertUtil.readCertificate(CRT_PATH + "serac-dev-test.crt"),
                new KeyUsageBits[] {
                    KeyUsageBits.DigitalSignature, KeyUsageBits.NonRepudiation,
                },
            },
            new Object[] {
                CertUtil.readCertificate(CRT_PATH + "login.live.com.crt"),
                new KeyUsageBits[] {
                    KeyUsageBits.DigitalSignature, KeyUsageBits.KeyEncipherment,
                },
            },
        };
    }
    ...
    public void certEncodedAsPkcs7(
            final X509Certificate certificate, final String expectedEncodedCert) {
        final String actualEncodedCertString =
                CertUtil.encodeCert(certificate, CertUtil.EncodeType.PKCS7);
        final X509Certificate decodedCert =
                CertUtil.decodeCertificate(
                        CertUtil.encodeCert(certificate, CertUtil.EncodeType.PKCS7).getBytes());
    ...
    public void testSubjectDN(final X509Certificate certificate, final String expectedResponse) {
        assertEquals(
                CertUtil.subjectDN(certificate, CertUtil.X500PrincipalFormat.RFC2253),
                expectedResponse);
    }

    @Test(dataProvider = "subject-dn-spaces")
    public void testSubjectDNWithSpaces(
            final X509Certificate certificate, final String expectedResponse) {
        assertEquals(
                CertUtil.subjectDN(certificate, CertUtil.X500PrincipalFormat.READABLE),
                expectedResponse);
    }
    ...
    public void testGenX509() {
        ...
        final X509Certificate x509Certificate =
                CertUtil.generateX509Certificate(
                        keyPair,
                        dn,
                        Date.from(expectedNotBefore),
                        Date.from(expectedNotAfter),
                        "SHA256WithRSA");

        assertEquals(
                truncateToSeconds(expectedNotBefore),
                truncateToSeconds(x509Certificate.getNotBefore().toInstant()));
        assertEquals(
                truncateToSeconds(expectedNotAfter),
