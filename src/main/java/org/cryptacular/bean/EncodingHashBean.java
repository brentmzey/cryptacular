...
public class EncodingHashBean extends AbstractHashBean implements HashBean<String> {
    ...
    /**
     * Creates a new instance that will not be salted. Delegates to {@link #EncodingHashBean(Spec,
     * Spec, int, boolean)}.
     *
     ...
     */
    public EncodingHashBean(
            final Spec<Codec> codecSpec, final Spec<Digest> digestSpec, final int iterations) {
    ...
    /**
     * Hashes the given data. If {@link #isSalted()} is true then the last parameter MUST be a byte
     * array containing the salt. The salt value will be appended to the encoded hash that is
     * returned.
     *
     ...
     *
     * @param hash Known encoded hash value. If the length of the hash bytes after decoding is
     *     greater than the length of the digest output, anything beyond the digest length is
