...
/**
 * Base class for all cipher beans. The base class assumes all ciphertext output will contain a
 * prepended {@link CiphertextHeaderV2} containing metadata that facilitates decryption.
 *
...
public abstract class AbstractCipherBean implements CipherBean {
    ...
    /**
     * Sets the keystore containing encryption/decryption key(s). The keystore must contain a {@link
     * SecretKey} entry whose alias is given by {@link #setKeyAlias(String)}, which will be used at
     * the encryption key. It may contain additional symmetric keys to support, for example, key
     * rollover where some existing ciphertexts have headers specifying a different key. In general
     * all keys used for outstanding ciphertexts should be contained in the keystore.
     *
    ...
    protected abstract byte[] process(
            org.cryptacular.CiphertextHeader header, boolean mode, byte[] input);
    ...
    /**
     * @return New ciphertext header for a pending encryption or decryption operation performed by
     *     this instance.
     */
