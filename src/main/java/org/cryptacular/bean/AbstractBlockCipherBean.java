...
public abstract class AbstractBlockCipherBean extends AbstractCipherBean {
    ...
    protected byte[] process(
            final org.cryptacular.CiphertextHeader header, final boolean mode, final byte[] input) {
     ...
     * @return Block cipher adapter that wraps an initialized block cipher that is ready for use in
     *     the given mode.
     */
    protected abstract BlockCipherAdapter newCipher(
            org.cryptacular.CiphertextHeader header, boolean mode);
}
