...
/**
 * {@link HashBean} implementation that uses the <em>bcrypt</em> algorithm for hashing. Hash strings
 * of the following format are supported: <br>
 * <code>
 ...
 * </code>
 *
 * <p>The encoding for salt and hash bytes is a variant of base-64 encoding without padding in the
 * following alphabet: <br>
...
public class BCryptHashBean implements HashBean<String> {
    ...
    /**
     * Compute a bcrypt hash of the form <code>
     * $2n$cost$xxxxxxxxxxxxxxxxxxxxxxxxyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy</code> given a salt and a
     * password.
     *
     * @param data A 2-element array containing salt and password. The salt may be encoded per the
     *     bcrypt standard or raw bytes.
     * @return An encoded bcrypt hash, <code>yyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy</code> in the
     *     specification above.
    ...
    public String hash(final Object... data) throws CryptoException {
        if (data.length != 2) {
            throw new IllegalArgumentException(
                    "Expected exactly two elements in data array but got " + data.length);
        }
        return encode(BCrypt.generate(password(version, data[1]), salt(data[0]), cost), 23);
    }

    /**
     * Compares a bcrypt hash of the form <code>
     * $2n$cost$xxxxxxxxxxxxxxxxxxxxxxxxyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy</code> with the computed
     * hash from the given password. The bcrypt algorithm parameters are derived from the reference
     * bcrypt hash string.
     *
    ...
    public boolean compare(final String hash, final Object... data)
        ...
        if (data.length != 1) {
            throw new IllegalArgumentException(
                    "Expected exactly one element in data array but got " + data.length);
        }
        final BCryptParameters params = new BCryptParameters(hash);
        final byte[] computed =
                BCrypt.generate(
                        password(params.getVersion(), data[0]), params.getSalt(), params.getCost());
        for (int i = 0; i < 23; i++) {
    ...
    private static String encode(final byte[] bytes, final int length) {
        final Encoder encoder =
                new Base64Encoder.Builder().setAlphabet(ALPHABET).setPadding(false).build();
    ...
    private static byte[] decode(final String input, final int length) {
        final Decoder decoder =
                new Base64Decoder.Builder().setAlphabet(ALPHABET).setPadding(false).build();
        ...
        if (output.limit() != length) {
            throw new IllegalArgumentException(
                    "Input is not of the expected size: " + output.limit() + "!=" + length);
        }
    ...
    /**
     * Converts an input object into a password as an array of UTF-8 bytes. A null terminator is
    ...
    /**
     * Handles encoding and decoding a bcrypt hash of the form <code>
     * $2n$cost$xxxxxxxxxxxxxxxxxxxxxxxxyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy</code>.
     */
    public static class BCryptParameters {
         ...
         *
         * @param bCryptString bcrypt hash of the form <code>
         *     $2n$cost$xxxxxxxxxxxxxxxxxxxxxxxxyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy</code>
         */
        protected BCryptParameters(final String bCryptString) {
            if (!bCryptString.startsWith("$2")) {
                throw new IllegalArgumentException(
                        "Expected bcrypt hash of the form $2n$cost$salthash");
            }
         ...
         *
         * @return Bcrypt hash of the form <code>
         *     $2n$cost$xxxxxxxxxxxxxxxxxxxxxxxxyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy</code>.
         */
        public String encode() {
            return '$'
                    ...
                    + cost
                    + '$'
                    + BCryptHashBean.encode(salt, 16)
                    + BCryptHashBean.encode(hash, 23);
        }

        /**
         * Produces an encoded bcrypt hash string from bcrypt parameters and a provided hash string.
         *
         * @param hash Encoded bcrypt hash bytes; e.g. the value produced from {@link
         *     #hash(Object...)}.
         * @return Bcrypt hash of the form <code>
         *     $2n$cost$xxxxxxxxxxxxxxxxxxxxxxxxyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy</code>.
         */
