...
public final class CipherUtil {
    ...
    /**
     * Encrypts data using an AEAD cipher. A {@link CiphertextHeaderV2} is prepended to the
     * resulting ciphertext and used as AAD (Additional Authenticated Data) passed to the AEAD
     * cipher.
     *
     ...
     * @return Concatenation of encoded {@link CiphertextHeaderV2} and encrypted data that
     *     completely fills the returned byte array.
     * @throws CryptoException on encryption errors.
     */
    public static byte[] encrypt(
            final AEADBlockCipher cipher, final SecretKey key, final Nonce nonce, final byte[] data)
            throws CryptoException {
        final byte[] iv = nonce.generate();
        final byte[] header = new CiphertextHeaderV2(iv, "1").encode(key);
        cipher.init(
                true,
                new AEADParameters(new KeyParameter(key.getEncoded()), MAC_SIZE_BITS, iv, header));
     ...
     * @param output Output stream that receives a {@link CiphertextHeaderV2} followed by ciphertext
     *     data produced by the AEAD cipher in encryption mode.
     * @throws CryptoException on encryption errors.
     ...
     * @param cipher AEAD cipher.
     * @param key Encryption key.
     * @param data Ciphertext data containing a prepended {@link CiphertextHeaderV2}. The header is
    ...
    public static byte[] decrypt(
            final AEADBlockCipher cipher, final SecretKey key, final byte[] data)
        ...
        cipher.init(
                false,
                new AEADParameters(
                        new KeyParameter(key.getEncoded()), MAC_SIZE_BITS, nonce, hbytes));
     ...
     * @param cipher AEAD cipher.
     * @param key Encryption key.
     * @param input Input stream containing a {@link CiphertextHeaderV2} followed by ciphertext
     *     data. The header is treated as AAD input to the cipher that is verified during
     *     decryption.
     * @param output Output stream that receives plaintext produced by block cipher in decryption
     *     mode.
     * @throws CryptoException on encryption errors.
     * @throws EncodingException on decoding cyphertext header.
     * @throws StreamException on IO errors.
    ...
    /**
     * Encrypts data using the given block cipher with PKCS5 padding. A {@link CiphertextHeaderV2}
     * is prepended to the resulting ciphertext.
     *
     * @param cipher Block cipher.
     * @param key Encryption key.
     * @param nonce IV generator. Callers must take care to ensure that the length of generated IVs
     *     is equal to the cipher block size.
     * @param data Plaintext data to be encrypted.
     * @return Concatenation of encoded {@link CiphertextHeaderV2} and encrypted data that
     *     completely fills the returned byte array.
     * @throws CryptoException on encryption errors.
     */
    public static byte[] encrypt(
            final BlockCipher cipher, final SecretKey key, final Nonce nonce, final byte[] data)
            throws CryptoException {
        final byte[] iv = nonce.generate();
        final byte[] header = new CiphertextHeaderV2(iv, "1").encode(key);
        final PaddedBufferedBlockCipher padded =
                new PaddedBufferedBlockCipher(cipher, new PKCS7Padding());
     ...
     * org.cryptacular.CiphertextHeader} is prepended to the resulting ciphertext.
     *
     * @param cipher Block cipher.
     * @param key Encryption key.
     * @param nonce IV generator. Callers must take care to ensure that the length of generated IVs
     *     is equal to the cipher block size.
     * @param input Input stream containing plaintext data.
     * @param output Output stream that receives ciphertext produced by block cipher in encryption
     *     mode.
     * @throws CryptoException on encryption errors.
     * @throws StreamException on IO errors.
     */
    ...
    public static byte[] decrypt(final BlockCipher cipher, final SecretKey key, final byte[] data)
        ...
        final PaddedBufferedBlockCipher padded =
                new PaddedBufferedBlockCipher(cipher, new PKCS7Padding());
        padded.init(
                false, new ParametersWithIV(new KeyParameter(key.getEncoded()), header.getNonce()));
     ...
     * @param cipher Block cipher.
     * @param key Encryption key.
     * @param input Input stream containing a {@link org.cryptacular.CiphertextHeader} followed by
     *     ciphertext data.
     * @param output Output stream that receives plaintext produced by block cipher in decryption
     *     mode.
     * @throws CryptoException on encryption errors.
    ...
    /**
     * Decodes the ciphertext header at the start of the given byte array. Supports both original
     * (deprecated) and v2 formats.
     *
    ...
    /**
     * Decodes the ciphertext header at the start of the given input stream. Supports both original
     * (deprecated) and v2 formats.
     *
        ...
        try {
            ...
            try {
                ...
                throw new StreamException(
                        "Stream error trying to process old header format: " + ioe.getMessage());
            }
     ...
     * @return Concatenation of encoded header and encrypted data that completely fills the returned
     *     byte array.
     */
    private static byte[] encrypt(
            final BlockCipherAdapter cipher, final byte[] header, final byte[] data) {
    ...
    private static byte[] decrypt(
            final BlockCipherAdapter cipher, final byte[] data, final int inOff) {
    ...
    /**
     * Performs encryption or decryption on the given input stream based on the underlying cipher
     * mode and writes the result to the given output stream.
     *
     * @param cipher Adapter for either a block or AEAD cipher.
    ...
    private static void process(
            final BlockCipherAdapter cipher, final InputStream input, final OutputStream output) {
