...
public final class HashUtil {
    ...
    /**
     * Computes the hash of the given data using the given algorithm. A salted hash may be produced
     * as follows:
     *
     * <pre>
     ...
     * @param data Data to hash. Supported types are <code>byte[]</code>, {@link CharSequence}
     *     ,{@link InputStream}, and {@link Resource}. Character data is processed in the <code>
     *     UTF-8</code> character set; if another character set is desired, the caller should
    ...
    /**
     * Computes the iterated hash of the given data using the given algorithm. The following example
     * demonstrates a typical usage pattern, a salted hash with 10 rounds:
     *
     * <pre>
     * // data is a byte array containing raw data to digest
     * final byte[] salt = new RBGNonce(16).generate();
     ...
     *
     * @param digest Hash algorithm.
     * @param hash Hash to compare with. If the length of the array is greater than the length of
     *     the digest output, anything beyond the digest length is considered salt data that is
     *     hashed <strong>after</strong> the input data.
     * @param iterations Number of hash rounds.
     * @param data Data to hash.
     * @return True if the hash of the data under the given digest is equal to the hash, false
     *     otherwise.
     * @throws CryptoException on hash computation errors.
     * @throws StreamException on stream IO errors.
     */
    ...
    /**
     * Digests the data in the given stream. Note this method does not finalize the digest process
     * by calling {@link Digest#doFinal(byte[], int)}.
     *
