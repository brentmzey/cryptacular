 ...
 *
 * <ul>
 *   <li>Len - Total header length in bytes (4-byte integer)
 *   <li>NonceLen - Nonce length in bytes (4-byte integer)
 *   <li>Nonce - Nonce bytes (variable length)
 *   <li>KeyNameLen (OPTIONAL) - Key name length in bytes (4-byte integer)
 *   <li>KeyName (OPTIONAL) - Key name encoded as bytes in platform-specific encoding (variable
 *       length)
 * </ul>
 *
 * <p>The last two fields are optional and provide support for multiple keys at the encryption
 * provider. A common case for multiple keys is key rotation; by tagging encrypted data with a key
 * name, an old key may be retrieved by name to decrypt outstanding data which will be subsequently
    ...
    public CiphertextHeader(final byte[] nonce, final String keyName) {
        if (nonce.length > MAX_NONCE_LEN) {
            throw new IllegalArgumentException(
                    "Nonce exceeds size limit in bytes (" + MAX_NONCE_LEN + ")");
        }
        if (keyName != null) {
            if (ByteUtil.toBytes(keyName).length > MAX_KEYNAME_LEN) {
                throw new IllegalArgumentException(
                        "Key name exceeds size limit in bytes (" + MAX_KEYNAME_LEN + ")");
            }
        }
        this.nonce = nonce;
            ...
            try {
                ...
                if (keyLen > MAX_KEYNAME_LEN) {
                    throw new EncodingException(
                            "Bad ciphertext header: maximum key length exceeded");
                }
                b = new byte[keyLen];
                bb.get(b);
