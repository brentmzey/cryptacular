 ...
 * </pre>
 *
 * <p>Where fields are defined as follows:
 *
 * <ul>
 *   <li>Version - Header version format as a negative number (4-byte integer). Current version is
 *       -2.
 *   <li>KeyName - Symbolic key name encoded as UTF-8 bytes (variable length)
 *   <li>0 - Null byte signifying the end of the symbolic key name
 *   <li>NonceLen - Nonce length in bytes (1-byte unsigned integer)
 *   <li>Nonce - Nonce bytes (variable length)
 *   <li>HMAC - HMAC-256 over preceding fields (32 bytes)
 * </ul>
 *
 * <p>The last two fields provide support for multiple keys at the encryption provider. A common
 * case for multiple keys is key rotation; by tagging encrypted data with a key name, an old key may
 * be retrieved by name to decrypt outstanding data which will be subsequently re-encrypted with a
 * new key.
 *
...
public class CiphertextHeaderV2 extends CiphertextHeader {
     ...
     * @param keyLookup Function used to look up the secret key from the symbolic key name in the
     *     header.
     * @return Decoded header.
     * @throws EncodingException when ciphertext header cannot be decoded.
     */
    public static CiphertextHeaderV2 decode(
     ...
     */
    public static CiphertextHeaderV2 decode(
            final InputStream input, final Function<String, SecretKey> keyLookup)
            throws EncodingException, StreamException {
        return decodeInternal(
                input,
                keyLookup,
                ByteUtil::readInt,
                CiphertextHeaderV2::readByte,
                CiphertextHeaderV2::readInto);
    }
        ...
        try {
            ...
            if (key == null) {
                throw new IllegalStateException(
                        "Symbolic key name mentioned in header was not found");
            }
    ...
    private static boolean arraysEqual(
            final byte[] a, final int aOff, final byte[] b, final int bOff, final int length) {
