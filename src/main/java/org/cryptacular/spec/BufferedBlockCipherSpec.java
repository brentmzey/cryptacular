...
/**
 * Describes a block cipher in terms of a (algorithm, mode, padding) tuple and provides a facility
 * to create a new instance of the cipher via the {@link #newInstance()} method.
 *
...
public class BufferedBlockCipherSpec implements Spec<BufferedBlockCipher>, Serializable {
    ...
    public static final Pattern FORMAT =
            Pattern.compile("(?<alg>[A-Za-z0-9_-]+)/(?<mode>\\w+)/(?<padding>\\w+)");
    ...
    public BufferedBlockCipherSpec(
            final String algName, final String cipherMode, final String cipherPadding) {
     ...
     *
     * @return Padding scheme algorithm, e.g. PKCS5Padding. The following names are equivalent for
     *     no padding: NULL, Zero, None.
     */
    ...
    /**
     * Parses a string representation of a buffered block cipher specification into an instance of
     * this class.
     *
     * @param specification Block cipher specification of the form <code>algorithm/mode/padding
     *     </code>.
