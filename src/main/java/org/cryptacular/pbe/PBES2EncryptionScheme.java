...
public class PBES2EncryptionScheme extends AbstractEncryptionScheme {
    ...
    private static final Map<ASN1ObjectIdentifier, DigestSpec> HMAC_ID_TO_DIGEST_SPEC_MAP =
            new HashMap<>();
    ...
    static {
        HMAC_ID_TO_DIGEST_SPEC_MAP.put(
                PKCSObjectIdentifiers.id_hmacWithSHA1, new DigestSpec("SHA1"));
        HMAC_ID_TO_DIGEST_SPEC_MAP.put(
                PKCSObjectIdentifiers.id_hmacWithSHA256, new DigestSpec("SHA256"));
        HMAC_ID_TO_DIGEST_SPEC_MAP.put(
                PKCSObjectIdentifiers.id_hmacWithSHA512, new DigestSpec("SHA512"));
        HMAC_ID_TO_DIGEST_SPEC_MAP.put(
                NISTObjectIdentifiers.id_hmacWithSHA3_256, new DigestSpec("SHA3", 256));
        HMAC_ID_TO_DIGEST_SPEC_MAP.put(
                NISTObjectIdentifiers.id_hmacWithSHA3_384, new DigestSpec("SHA3", 384));
        HMAC_ID_TO_DIGEST_SPEC_MAP.put(
                NISTObjectIdentifiers.id_hmacWithSHA3_512, new DigestSpec("SHA3", 512));
    }
    ...
    public PBES2EncryptionScheme(final PBES2Parameters params, final char[] password) {
        final PBKDF2Params kdfParams =
                PBKDF2Params.getInstance(params.getKeyDerivationFunc().getParameters());
    ...
    private void initCipher(
        ...
        switch (alg) {
                ...
                if (rc2Params.size() > 1) {
                    cipherParameters =
                            new RC2Parameters(
                                    ...
                                    ASN1Integer.getInstance(rc2Params.getObjectAt(0))
                                            .getValue()
                                            .intValue());
            ...
            case RC5:
                ...
                final int rounds =
                        ASN1Integer.getInstance(rc5Params.getObjectAt(1)).getValue().intValue();
                final int blockSize =
                        ASN1Integer.getInstance(rc5Params.getObjectAt(2)).getValue().intValue();
                if (blockSize == 64) {
                    setCipher(
                            new PaddedBufferedBlockCipher(
                                    CBCBlockCipher.newInstance(new RC564Engine()),
                                    new PKCS7Padding()));
                } else if (blockSize == 32) {
                    setCipher(
                            new PaddedBufferedBlockCipher(
                                    CBCBlockCipher.newInstance(new RC532Engine()),
                                    new PKCS7Padding()));
                } else {
                    throw new IllegalArgumentException("Invalid RC5 block size: " + blockSize);
                }
                cipherParameters =
                        new RC5Parameters(((KeyParameter) cipherParameters).getKey(), rounds);
                if (rc5Params.size() > 3) {
