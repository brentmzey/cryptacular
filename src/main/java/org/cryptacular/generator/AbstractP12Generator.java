...
public abstract class AbstractP12Generator implements P12Generator {
    ...
    public PKCS12PfxPdu generate(
        ...
        try {
            ...
            final PKCS12SafeBagBuilder keyBagBuilder =
                    new JcaPKCS12SafeBagBuilder(key, keyOutputEncryptor(password));
            keyBagBuilder.addBagAttribute(
                    PKCSObjectIdentifiers.pkcs_9_at_friendlyName, new DERBMPString(alias));
            keyBagBuilder.addBagAttribute(
                    PKCSObjectIdentifiers.pkcs_9_at_localKeyId,
                    extUtils.createSubjectKeyIdentifier(certificates[0].getPublicKey()));
            certBags[0] =
                    new JcaPKCS12SafeBagBuilder(certificates[0])
                            .addBagAttribute(
                            ...
                            .addBagAttribute(
                                    ...
                                    extUtils.createSubjectKeyIdentifier(
                                            certificates[0].getPublicKey()))
                            .build();
            for (int i = 1; i < certificates.length; i++) {
                label = "ca-cert-" + i;
                certBags[i] =
                        new JcaPKCS12SafeBagBuilder(certificates[i])
                                .addBagAttribute(
                                        PKCSObjectIdentifiers.pkcs_9_at_friendlyName,
                                        new DERBMPString(label))
                                .build();
            }
            ...
            final PKCS12MacCalculatorBuilder macCalculatorBuilder =
                    new BcPKCS12MacCalculatorBuilder(
                                    (ExtendedDigest) digestSpec.newInstance(),
                                    new AlgorithmIdentifier(
                                            getDigestAlgorithmId(), DERNull.INSTANCE))
    ...
    @Override
    public PKCS12PfxPdu generate(
            final char[] password, final PrivateKey key, final X509Certificate... certificates) {
    ...
    /**
     * Builds a new output encryptor that performs password-based encryption on keys in the P12
     * file.
     *
    ...
    /**
     * Builds a new output encryptor that performs password-based encryption on encrypted data in
     * the P12 file.
     *
     * @param password Password tha will the basis of an encryption key.
     * @return Output encryptor.
