...
/**
 * RBG-based nonce generation strategy that uses a RBG component to produce values for the
 * invocation field as described in NIST <a
 * href="http://csrc.nist.gov/publications/nistpubs/800-38D/SP-800-38D.pdf">SP-800-38D</a>, section
 * 8.2.2.
 *
 * <p><strong>NOTE:</strong> users of this class are responsible for counting number of invocations
 * and enforcing the constraints described in section 8.3; namely the following:
 *
 * <blockquote>
 *
...
public class RBGNonce implements Nonce {
    ...
    /**
     * Creates a new instance that produces 12-bytes (96-bits) of random data; that is, the fixed
     * field of the nonce is null.
     */
    public RBGNonce() {
        this(12);
    }
    ...
    /**
     * Creates a new instance that produces length bytes of random data; that is, the fixed field of
     * the nonce is null.
     *
    ...
    private static SP80090DRBG newRBG(final int length, final byte[] domain) {
        return new HashSP800DRBG(
                new SHA256Digest(),
                length,
                NonceUtil.randomEntropySource(length),
                domain,
