...
/**
 * Container for the output of a salted hash operation that includes both the digest output and salt
 * value.
 *
...
public class SaltedHash {
    ...
    /**
     * Creates a new instance from byte input that contains the concatenation of digest output and
     * salt.
     *
    ...
    public byte[] getSalt(final int n) {
        if (n > salt.length) {
            throw new IllegalArgumentException(
                    "Requested size exceeded length: " + n + ">" + salt.length);
        }
